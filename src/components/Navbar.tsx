  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white/95 backdrop-blur-md border-b border-brand-beige shadow-xs">
      ...
      <div className="bg-brand-red text-white text-xs py-1 px-4 text-center font-medium tracking-wide hidden sm:block">
        ...
        <span className="inline-flex items-center gap-2">
          <MapPin className="w-3.5 h-3.5" /> Mangunreja, Tasikmalaya (Bisa Pesan Antar)
        </span>
      </div>
        ...
        <nav className="hidden md:flex items-center gap-1">
          ...
          {onOpenAdmin && (
            <button
              onClick={onOpenAdmin}
              id="nav-admin-btn"
              className="ml-2 px-3 py-1 text-xs border border-brand-beige hover:border-brand-orange text-stone-500 hover:text-brand-orange rounded-md font-mono transition-colors cursor-pointer"
            >
              Admin Panel
            </button>
          )}
          ...
          <div className="pt-2 border-t border-brand-beige flex flex-col gap-2">
            {onOpenAdmin && (
              <button
                ...
                className="w-full text-center py-2.5 bg-brand-cream border border-brand-beige text-stone-600 font-mono text-sm rounded-xl hover:bg-stone-50"
              >
                Admin Panel
              </button>
            )}
